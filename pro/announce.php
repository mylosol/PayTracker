...
include ('../include/variables.php');	  
	...
	$conn->query("UPDATE account SET announce = '0' WHERE account.id = ".$id." LIMIT 1;") or die ('<center><h1>Unable to set announcement as viewed</h1><h3>Something Has Gone <span style="color:#F00;">Catastrophically</span> Wrong!</h3><h3>Wait a bit and try again</h3></center>');
...
<?php
		$getAnnounce = $conn->query("SELECT * FROM announce;");
		while ($row = $getAnnounce->fetch_assoc()) {
		$title = $row["title"];
		$content = $row["content"];
?>

		<h1 align="center"><?php echo $title; ?></h1>
        <h3 align="center"><?php echo $content; ?></h3>
        <br /> 



<?php
		}//end while ($row= $getAnnounce->fetch_assco())
?>

<center>          
<center>          
<form action="http://'.$_SERVER['SERVER_NAME'].'/pro/tracking.php">
    <input class="myButton threeQuarterB blueB" type="submit" value="Okay, I Got It!" />
</form></center>
