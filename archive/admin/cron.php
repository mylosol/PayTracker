	  ...
	  $getWeeklyTotal = $conn->query("SELECT * FROM totals WHERE id = '".$id."' LIMIT 1;");
		 while ($row = $getWeeklyTotal->fetch_assoc()) { 
		  ...
		  $yearDiffTotal = $yearDiff + $difference;	
			...
			$conn->query("UPDATE `totals` SET `curWeek` = '0' WHERE `totals`.`id` = ".$id.";");
			$conn->query("UPDATE `totals` SET `curDiff` = '0' WHERE `totals`.`id` = ".$id.";");
			
			  if ($curYear != $ydy) {
			  $conn->query("UPDATE totals SET yearDiff = 0.00 WHERE totals.id= ".$id." LIMIT 1;<br />");
			  $conn->query("UPDATE totals SET ydy = ".$curYear." WHERE totals.id= ".$id." LIMIT 1;<br />");
			  } else {
			  $conn->query("UPDATE totals SET yearDiff = ".number_format($yearDiffTotal,2)." WHERE totals.id= ".$id." LIMIT 1;<br />");
			  }//end if ($curYear != $ydy)
...
include ('../include/paidthroughEmail.php');
