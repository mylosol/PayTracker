...
$getMiles = $_POST['m'];
$anchor = $_GET['a'];
$getNumQuery = "SELECT * FROM `PanamaPay` ORDER BY `id` DESC";
$result = $conn->query($getNumQuery);
	...
	if (isset($getMiles)) {
		...
		$conn->query("INSERT INTO `PanamaPay` (`id`, `miles`, `rate`) VALUES (NULL, '".$getMiles."', '".$getPay."')");
		header("Location: http://".$_SERVER['SERVER_NAME']."/UpdatePanamaPay.php");
...
<div id="wrapper" class="largeContain" style="margin-top:30px;">
  <div id="Top" class="add-load-contain">
    ...
    <center>
<form method="post" action="UpdatePanamaPay.php">
